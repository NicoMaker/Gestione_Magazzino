...
async function loadPDFJS() {
  return new Promise((resolve, reject) => {
    ...
    script.onerror = () => reject(new Error("Impossibile caricare la libreria PDF.js"));
        ...
        for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
          ...
          textContent.items.forEach((item, index) => {
            ...
            if (index > 0 && lastY === currentY) {
              ...
              if (item.transform[4] - (prevItem.transform[4] + prevItem.width) > 2) pageText += " ";
            }
            pageText += item.str;
            lastY = currentY;
          });
...
function extractScarichiFromPDF(text, date) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].toUpperCase().includes("RICAMBI")) { ricambiStartIndex = i; break; }
  }
  if (ricambiStartIndex === -1) return scarichi;

  const headerKeywords = ["CODICE","RICAMBIO","DESCRIZIONE","QUANTITA","PREZZO","IVA","ESCLUSA","QUANTITÀ"];
  let dataStartIndex = ricambiStartIndex + 1;
  for (let i = ricambiStartIndex + 1; i < Math.min(ricambiStartIndex + 10, lines.length); i++) {
    if (headerKeywords.some((kw) => lines[i].toUpperCase().includes(kw))) dataStartIndex = i + 1;
    else break;
  }

  const stopKeywords = ["MANODOPERA","TOTALE","IVA COMPRESA","DESCRIZIONE LAVORAZIONI","LAVORAZIONI"];
  for (let i = dataStartIndex; i < lines.length; i++) {
    const line = lines[i].trim();
    if (!line) continue;
    if (stopKeywords.some((kw) => line.toUpperCase().includes(kw))) break;
    const parsed = parseRicamboLine(line);
    if (parsed) scarichi.push({ code: parsed.code, quantity: parsed.quantity, date });
  }
  return scarichi;
}
...
function parseRicamboLine(line) {
  ...
  if (match1) {
    ...
    if (isValidCode(match1[2]) && isValidQuantity(qty)) return { code: match1[2], quantity: qty };
  }
  const pattern2 = /^(\d+)\s+(\d+(?:[,\.]\d{1,2})?)$/;
  const match2 = cleanLine.match(pattern2);
  if (match2) {
    const qty = parseFloat(match2[2].replace(",", "."));
    if (isValidCode(match2[1]) && isValidQuantity(qty)) return { code: match2[1], quantity: qty };
  }
  const words = cleanLine.split(/\s+/);
  if (words.length >= 2) {
    const qty = parseFloat(words[words.length - 1].replace(",", "."));
    if (isValidCode(words[0]) && isValidQuantity(qty)) return { code: words[0], quantity: qty };
  }
  return null;
}

function isValidCode(code)     { return /^[A-Z0-9\-_\/]{1,50}$/i.test(code); }
function isValidQuantity(qty)  { return !isNaN(qty) && qty > 0 && qty <= 9999; }
...
function normalizeDate(dateStr) {
  ...
  if (parts.length === 3) {
    if (parts[0].length === 4) return dateStr.replace(/\./g, "-").replace(/\//g, "-");
...
async function checkProductExists(code) {
  try {
    ...
    return prodotti.find((p) => p.nome.toUpperCase() === code.toUpperCase()) || null;
  } catch { return null; }
}

async function processScarichi(scarichi, nomeFilePdf) {
  const results = { success: [], failed: [], notFound: [], insufficientStock: [] };
  let prodottiDB = [];
  try {
    prodottiDB = await fetch(`${API_URL}/prodotti`).then((r) => r.json());
  } catch (e) {
    results.failed.push({ code: "–", reason: "Errore caricamento prodotti dal server" });
    return results;
  }
  ...
  for (const s of scarichi) {
    ...
    if (!codice) continue;
    if (!aggregati[codice]) aggregati[codice] = { code: codice, quantity: 0, date: s.date };
    aggregati[codice].quantity += s.quantity;
  }

  const scarichiDaInviare = [];
  for (const [codice, agg] of Object.entries(aggregati)) {
    const prodotto = prodottiDB.find((p) => p.nome.trim().toUpperCase() === codice);
    if (!prodotto) {
      results.notFound.push({ code: codice, quantity: agg.quantity, date: agg.date, reason: "Prodotto non trovato nel database" });
      continue;
    }
    scarichiDaInviare.push({ codice, prodotto_id: prodotto.id, quantita: parseFloat(agg.quantity.toFixed(2)), data_movimento: agg.date });
  }
  ...
  try {
    ...
    if (!res.ok) { results.failed.push({ code: "–", reason: data.error || "Errore sconosciuto" }); return results; }
    for (const s of data.risultati.success)            results.success.push({ code: s.codice, nome: s.codice, quantity: s.quantita, date: s.data_movimento });
    for (const s of data.risultati.insufficientStock)  results.insufficientStock.push({ code: s.codice, nome: s.codice, quantity: s.quantita, available: s.disponibile, date: s.data_movimento, reason: s.reason });
    for (const s of data.risultati.failed)             results.failed.push({ code: s.codice || "–", reason: s.reason });
  } catch (e) {
...
async function handlePDFImport(file) {
  try {
    ...
    if (results.success.length > 0) { await loadMovimenti(); await loadProdotti(); }
    return results;
  } catch (error) {
...
function showImportResults(results) {
  ...
  const total = results.success.length + results.failed.length + results.notFound.length + results.insufficientStock.length;
  ...
  if (results.failed.length > 0) {
    ...
    results.failed.forEach((r) => { msg += `  • ${r.code}: ${r.reason}\n`; });
  }

  alert(msg);
  if (results.failed.length === 0 && results.notFound.length === 0 && results.insufficientStock.length === 0) {
    closeImportPDFModal();
  }
}
...
async function importaOrdineDaPdf(righePdf, dataOrdine, nomeFilePdf) {
  ...
  for (const mov of Object.values(aggregati)) {
    const prodotto = allProdotti.find((p) => String(p.codice) === String(mov.codice));
    if (!prodotto) { console.warn("Prodotto non trovato per codice", mov.codice); continue; }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      if (!file)                          { alert("⚠️ Seleziona un file PDF"); return; }
      if (file.type !== "application/pdf") { alert("⚠️ Il file deve essere un PDF"); return; }
      if (file.size > 10 * 1024 * 1024)  { alert("⚠️ Il file è troppo grande (max 10MB)"); return; }
      try { await handlePDFImport(file); } catch (error) { console.error("Errore import:", error); }
    });
  }
