...
async function loadMovimenti() {
  try {
    ...
    if (!res.ok) { movimenti = []; allMovimenti = []; renderMovimenti(); return; }
...
function renderMovimenti() {
  ...
  tbody.innerHTML = movimenti.map((m) => {
    ...
    const prezzoTotHtml = formatCurrency(m.prezzo_totale_movimento || 0).replace("€", `${prefix}€`);
...
function togglePrezzoField() {
  ...
  if (tipo === "carico") {
    prezzoGroup.style.display = "block"; prezzoInput.required = true;
    fatturaGroup.style.display = "block"; fatturaInput.required = true;
    if (docOptional) docOptional.textContent = "*";
    fornitoreGroup.style.display = "block"; fornitoreInput.required = true;
    if (fornitoreOptional) fornitoreOptional.textContent = "*";
  } else {
    prezzoGroup.style.display = "none"; prezzoInput.required = false; prezzoInput.value = "";
    fatturaGroup.style.display = "none"; fatturaInput.required = false; fatturaInput.value = "";
    if (docOptional) docOptional.textContent = "";
    fornitoreGroup.style.display = "none"; fornitoreInput.required = false; fornitoreInput.value = "";
    if (fornitoreOptional) fornitoreOptional.textContent = "";
  }
}
...
async function openMovimentoModal(movimento = null) {
  ...
  } catch { console.warn("⚠️ Impossibile ricaricare prodotti, uso cache"); }
...
async function showGiacenzaInfo(prodottoId) {
  ...
  if (prodotto) {
    ...
    const gi = document.getElementById("giacenzaInfo");
    if (el) el.textContent = `${prodotto.nome}${prodotto.marca_nome ? ` (${prodotto.marca_nome})` : ""} - Giacenza: ${formatQuantity(prodotto.giacenza || 0)} pz`;
...
async function deleteMovimento(id, prodottoNome, tipo) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      showAlertModal("Movimento eliminato con successo!", "Operazione Completata", "success");
      await loadMovimenti();
      await loadProdotti();
    } else {
...
document.getElementById("formMovimento").addEventListener("submit", async (e) => {
  ...
  const tipo        = document.getElementById("movimentoTipo").value;
  const quantita    = parseDecimalInput(document.getElementById("movimentoQuantita").value);
  const data_mov    = document.getElementById("movimentoData").value;

  let prezzo = null;
  if (tipo === "carico") prezzo = parseDecimalInput(document.getElementById("movimentoPrezzo").value);
  ...
  if (!prodotto_id || !tipo || !quantita || !data_mov) { alert("Compila tutti i campi obbligatori!"); return; }
  if (quantita <= 0) { alert("La quantità deve essere maggiore di 0!"); return; }
  if (tipo === "carico") {
    if (!prezzo || prezzo <= 0) { alert("Il prezzo deve essere maggiore di 0 per i carichi!"); return; }
    if (!fattura_doc || !fornitore) { alert("Documento e Fornitore sono obbligatori per i carichi!"); return; }
  }
  ...
  try {
    const res  = await fetch(url, {
      ...
      body: JSON.stringify({ prodotto_id, tipo, quantita: parseFloat(quantita.toFixed(2)), prezzo: prezzo ? parseFloat(prezzo.toFixed(2)) : null, data_movimento: data_mov, fattura_doc, fornitore })
    });
    const data = await res.json();
    if (res.ok) {
      if (typeof ignoreNextSocketUpdate === "function") ignoreNextSocketUpdate();
...
function setupProductSearch() {
  ...
  searchInput.addEventListener("input", (e) => {
    ...
    if (!searchTerm) { resultsCont.classList.remove("show"); resultsCont.innerHTML = ""; return; }
...
async function searchProducts() {
  ...
  if (!allProdotti || allProdotti.length === 0) {
    ...
    try {
      ...
      allProdotti = await res.json();
      prodotti = allProdotti;
    } catch { resultsCont.innerHTML = `<div class="search-no-results">Errore nel caricamento dei prodotti.</div>`; return; }
  }

  const filtered = allProdotti.filter((p) =>
    !searchTerm ||
    p.nome.toLowerCase().includes(searchTerm) ||
    (p.marca_nome || "").toLowerCase().includes(searchTerm) ||
    (p.descrizione || "").toLowerCase().includes(searchTerm)
...
function _renderProductSearchResults(filtered, searchTerm) {
  ...
  resultsCont.querySelectorAll(".search-result-item").forEach((item) => {
    item.addEventListener("click", function () {
      selectProduct(this.dataset.id, this.dataset.nome, this.dataset.marca, this.dataset.giacenza);
    });
  });
  resultsCont.classList.add("show");
}
